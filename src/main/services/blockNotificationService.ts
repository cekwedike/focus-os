...
import { BLOCK_WARNING_ACTIONS } from '@shared/notifications/notificationActions'
...
function firePreCompletionNotification(block: DailyScheduleRow, threshold: PreCompletionThreshold): void {
  ...
  notify({
    ...
    message: text,
    urgency: threshold <= 5 ? 'high' : 'normal',
    persistent: false,
    dedupeKey: `block_warning:${block.id}:${threshold}`,
    actions: BLOCK_WARNING_ACTIONS,
    metadata: { blockId: block.id, threshold },
  })
}
