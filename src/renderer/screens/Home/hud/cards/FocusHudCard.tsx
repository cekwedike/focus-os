...
export function FocusHudCard(): React.JSX.Element {
  ...
  const completionBars = useMemo((): HudBarDatum[] => {
  ...
  return (
    <HudCard accent="mint" expanded={expanded} onClick={() => setExpanded((o) => !o)}>
      <div className="flex items-start justify-between gap-2">
        ...
        <HudRingGauge value={hasScore ? score : 0} size={64} active={hasScore} />
      </div>
      ...
      {expanded ? (
        ...
        >
          {completionBars.length > 0 ? (
            <HudMiniBars data={completionBars} height={48} maxValue={100} />
          ) : (
